...
router.post('/generate', async (req, res) => {
    try {
        ...
        const projectBuild = new Generation_1.ProjectBuild({
        ...
        processProjectBuild(projectBuild._id.toString());
        res.json({
            success: true,
            generationId: projectBuild._id.toString(),
...
router.get('/status/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const projectBuild = await Generation_1.ProjectBuild.findById(id);
        if (!projectBuild) {
...
router.get('/history', async (req, res) => {
    try {
        ...
        const projectBuilds = await Generation_1.ProjectBuild.find()
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(Number(limit))
            .select('-accessToken'); // Exclude sensitive data
        const total = await Generation_1.ProjectBuild.countDocuments();
        res.json({
            success: true,
...
router.post('/push-dockerfile', async (req, res) => {
    try {
        ...
        const projectBuild = await Generation_1.ProjectBuild.findById(generationId);
        if (!projectBuild) {
            return res.status(404).json({
                error: 'Project build not found'
            });
        ...
        const repositoryProvider = new GitHubService_1.RepositoryDataProvider(projectBuild.accessToken);
        const success = await repositoryProvider.commitDockerfileToRepository(projectBuild.repositoryUrl, projectBuild.generatedDockerfile, commitMessage || 'Add Dockerfile generated by DockGen AI');
        if (success) {
            res.json({
                success: true,
...
router.get('/images', async (req, res) => {
    try {
        const containerBuildEngine = new DockerService_1.ContainerBuildEngine();
...
async function processProjectBuild(buildId) {
    try {
        const projectBuild = await Generation_1.ProjectBuild.findById(buildId);
        if (!projectBuild)
            return;
        ...
        const analysisEngine = new AIAgentService_1.IntelligentAnalysisEngine();
