...
exports.IntelligentAnalysisEngine = void 0;
const google_genai_1 = require("@langchain/google-genai");
class IntelligentAnalysisEngine {
    constructor() {
        this.languageModel = new google_genai_1.ChatGoogleGenerativeAI({
            model: 'gemini-2.0-flash',
            temperature: 0.1,
            apiKey: process.env.GEMINI_API_KEY
        });
    }
    async analyzeProjectTechnologies(repoData) {
        try {
...
Files: ${repoData.files.map(f => f.name).join(', ')}`;
    ...
    async createContainerConfiguration(repoData, detectedTechnologies) {
        try {
...
Generate a production-ready, runnable Dockerfile that will successfully build and run.`;
            const prompt = `${systemPrompt}\n\n${userPrompt}`;
            const response = await this.languageModel.invoke(prompt);
            const dockerfileContent = typeof response === 'string' ? response : response.content;
    ...
    extractTechnologyNames(response) {
        try {
    ...
    performBasicTechnologyDetection(repoData) {
        const detectedTechnologies = ['JavaScript'];
    ...
    containsInvalidDockerSyntax(dockerfile) {
    ...
    createFallbackContainerConfiguration(repoData, detectedTechnologies) {
...
CMD ["${startScript}"]`;
    ...
    sanitizeDockerfileContent(dockerfile) {
...
exports.IntelligentAnalysisEngine = IntelligentAnalysisEngine;
