...
export class IntelligentAnalysisEngine {
  private languageModel: ChatGoogleGenerativeAI;

  constructor() {
    this.languageModel = new ChatGoogleGenerativeAI({
  ...
  async analyzeProjectTechnologies(repoData: RepositoryData): Promise<string[]> {
    try {
...
Files: ${repoData.files.map(f => f.name).join(', ')}`;
      ...
      const response = await this.languageModel.invoke(prompt);
      const content = typeof response === 'string' ? response : (response as any).content;

      // Parse the response to extract technology names
      const detectedTechnologies = this.extractTechnologyNames(content);
      ...
      if (error instanceof Error && error.message.includes('quota')) {
        ...
        return this.performBasicTechnologyDetection(repoData);
      }
  ...
  async createContainerConfiguration(repoData: RepositoryData, detectedTechnologies: string[]): Promise<string> {
    try {
...
Return ONLY the Dockerfile content, no explanations or markdown formatting.`;
      ...
      const userPrompt = `Repository: ${repoData.fullName}
Tech Stack: ${detectedTechnologies.join(', ')}
Dependencies: ${repoData.dependencies?.join(', ') || 'None'}
Dev Dependencies: ${repoData.devDependencies?.join(', ') || 'None'}
...
Generate a production-ready, runnable Dockerfile that will successfully build and run.`;
      ...
      const prompt = `${systemPrompt}\n\n${userPrompt}`;
      const response = await this.languageModel.invoke(prompt);
      const dockerfileContent = typeof response === 'string' ? response : (response as any).content;

      const cleanedDockerfile = this.sanitizeDockerfileContent(dockerfileContent);
      
      // Validate the generated Dockerfile syntax
      if (this.containsInvalidDockerSyntax(cleanedDockerfile)) {
        console.warn('Generated Dockerfile has invalid syntax, using fallback');
        return this.createFallbackContainerConfiguration(repoData, detectedTechnologies);
      }
  ...
  private extractTechnologyNames(response: string): string[] {
    try {
  ...
  private performBasicTechnologyDetection(repoData: RepositoryData): string[] {
    const detectedTechnologies: string[] = ['JavaScript'];
    ...
    if (dependencies.includes('express') || devDependencies.includes('express')) {
      detectedTechnologies.push('Express');
    }
    ...
    if (repoData.files.some(f => f.name === 'yarn.lock')) {
      detectedTechnologies.push('Yarn');
    } else if (repoData.files.some(f => f.name === 'package-lock.json')) {
      detectedTechnologies.push('npm');
    }

    return detectedTechnologies;
  }

  private containsInvalidDockerSyntax(dockerfile: string): boolean {
  ...
  private createFallbackContainerConfiguration(repoData: RepositoryData, detectedTechnologies: string[]): string {
    const isReact = detectedTechnologies.includes('React');
    const isNext = detectedTechnologies.includes('Next.js');
    const isVue = detectedTechnologies.includes('Vue');
    const isAngular = detectedTechnologies.includes('Angular');
    const isExpress = detectedTechnologies.includes('Express');
    const hasTypeScript = detectedTechnologies.includes('TypeScript');
...
CMD ["${startScript}"]`;
  ...
  private sanitizeDockerfileContent(dockerfile: string): string {
